...
static double degree_minutes_to_decimal_degrees(double degree_minutes,
                                                char hemisphere) {
...
static uint8_t hex_to_nibble(char c) {
    if (c >= '0' && c <= '9')
        return c - '0';
    if (c >= 'A' && c <= 'F')
        return 10 + (c - 'A');  // subtract A(base16) add 10(base10)
    if (c >= 'a' && c <= 'f')
        return 10 + (c - 'a');  // incase ascii character is in lowercase
...
static void parse_rmc(GpsStats* debugger, GpsFix* fix, char* rmc_string,
    ...
    while (token) {
        if (field == 2)
            status = token[0];  // if field == 2, at the 3rd part of the rmc string, this represents status
        if (field == 3)
...
void gps_parser_feed(GpsStats* debugger, GpsFix* fix, const uint8_t* data,
    ...
    if (!debugger || !fix || !data || length == 0)  // check if data was even given
        return;

    for (size_t i = 0; i < length; i++) {
        char character_i = (char)data[i];

        if (character_i == '$') {  // found the start of a nmea string
        ...
        if (assembler.length < (NMEA_MAX_LEN - 1)) {
            assembler.sentence[assembler.length++] = character_i;
            assembler.sentence[assembler.length] = '\0';
        } else {  // TO PREVENT OVERFLOW: Abandon current string, wait for the next '$' (new nmea string)
        ...
        if (character_i == '\n') {       // \r\n is the escape sequence for nmea strings
            ...
            parse_sentence(debugger, fix, assembler.sentence,
                           message_timestamp);  // update the fix struct
                                                //			} else
