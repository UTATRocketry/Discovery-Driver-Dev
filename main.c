  ...
  while (1)
  {
	  ...
	  status = NEOM9N_getTime(&time, GPSData, sizeof(GPSData));
	   ...
	   if (status == 0) {
	     ...
	     //in CubeIDE, floats with printf must be manually enabled
	     sprintf((char*)tx_buff, "UTC Time: %02d:%02d:%05.2f\n\r", hour, minute, second);
	   } else {
	     //printf("Could not parse time from GPS data.\n");
	   ...
	   if (NEOM9N_getPosition(&latitude, &latHem, &longitude, &lonHem, GPSData,
	                          sizeof(GPSData)) == 0) {
		   sprintf((char*)tx_buff, "Latitude:  %.8f %c\n\rLongitude: %.8f %c\n\r", latitude, latHem, longitude, lonHem);
	   } else {
	   ...
	   if (NEOM9N_getAltitude(&altitude, GPSData) == 0) {
		   sprintf((char*)tx_buff, "Altitude: %.2f \n\r", altitude);
	   } else {
